...
db_long.reshape<- reshape(db_wide, direction="long",
                          ...
                          v.names=c("kilos"), #define name for column for the repeated measure
                          timevar="time", #define name for the column of time (repetition)
                          times=c("2000","2002"))  #define values for the "timevar"

head(db_long.reshape)

db_long.reshape<-db_long.reshape[order(db_long.reshape$ID), ]
#---------------------------------------------------
...
# to use the plot function we do not need objects to be in a data.frame: 
