...
BMI2<-factor(BMI, levels=c("low","high"), 
...
summary(dbp)
# Question 2 
# Disease is imported as character, 
# can we understand from the summary how many different diseases are in the data?

# 1. Would it be better if Disease were of a different type? If yes, which one?
factor(Disease)
summary(dbp)
...
# 4.Create a new variable Cancer that groups the Disease into Breast cancer and Others:
...
# Age is a continuous covariate. We would like to have an idea of the 
...
# 2. Calculate the mean of age by Cancer group 
...
# 3. Create a new variable in the data.frame for a categorical 
# variable for age with the median as cut-off for the two 
# categories. We can code it using two different functions in R:
...
# 4. Create a Table to count how many patients belong to each 
# age group.

table(dbp$AgeCat)

# 5. Show the proportion of patients by cancer group (use prop.table)
  
tab <- table(dbp$Cancer)
prop.table(tab)

# Question 4 Consider only patients with Breast cancer:
  
# 1. Subset patients with Breast cancer (use the function subset)
